...
namespace colmap {
namespace {
size_t TriangulateImage(const HybridMapperOptions& options, const Image& image,
                        HybridMapper* mapper) {
...
size_t FilterPoints(const HybridMapperOptions& options, HybridMapper* mapper) {
...
size_t FilterImages(const HybridMapperOptions& options, HybridMapper* mapper) {
...
size_t CompleteAndMergeTracks(const HybridMapperOptions& options,
                              HybridMapper* mapper) {
...
void AdjustGlobalBundle(const HybridMapperOptions& options,
                        HybridMapper* mapper) {
...
void IterativeLocalRefinement(const HybridMapperOptions& options,
                              const image_t image_id, HybridMapper* mapper) {
...
void IterativeGlobalRefinement(const HybridMapperOptions& options,
                               HybridMapper* mapper) {
...
HybridMapper::Options HybridMapperOptions::Mapper() const {
  HybridMapper::Options options = mapper;
...
IncrementalTriangulator::Options HybridMapperOptions::Triangulation() const {
...
BundleAdjustmentOptions HybridMapperOptions::LocalBundleAdjustment() const {
...
BundleAdjustmentOptions HybridMapperOptions::GlobalBundleAdjustment() const {
  BundleAdjustmentOptions options;
...
#endif  // CERES_VERSION_MAJOR
  options.print_summary = true;
  options.arkit_rotation_weight = ba_arkit_rotation_weight; 
  options.arkit_position_weight = ba_arkit_position_weight; 
  options.global_rotation_weight = ba_global_rotation_weight;
  options.depth_weight = ba_depth_weight;
  options.refine_focal_length = ba_refine_focal_length;
  options.refine_principal_point = ba_refine_principal_point;
  options.refine_extra_params = ba_refine_extra_params;
  options.min_num_residuals_for_multi_threading =
...
bool HybridMapperOptions::Check() const {
...
HybridMapperController::HybridMapperController(
    const HybridMapperOptions* options, const std::string& image_path,
    const std::string& depth_path, const std::string& database_path,
  ...
  RegisterCallback(GLOBAL_ROTATION_CALLBACK);
  RegisterCallback(GLOBAL_POSITION_CALLBACK);
  ...
  RegisterCallback(FINISH_RECONSTRUCTION_CALLBACK);
}
...
void HybridMapperController::Run() {
  ...
  HybridMapper::Options init_mapper_options = options_->Mapper();
...
bool HybridMapperController::LoadDatabase() {
  ...
  database_cache_.Load(database, image_path_, depth_path_, options_->image_json,
                       min_num_matches, options_->ignore_watermarks,
                       options_->arkit_view_graph, image_names);
  std::cout << std::endl;
  timer.PrintMinutes();
...
void HybridMapperController::Reconstruct(
    const HybridMapper::Options& init_mapper_options) {
  ...
  HybridMapper mapper(&database_cache_);
  ...
  for (int num_trials = 0; num_trials < options_->init_num_trials;
    ...
    PrintHeading1("Estimate global rotation");
    mapper.FilterInitialViewGraph(init_mapper_options.min_num_two_view_inliers);
    if (!mapper.EstimateGlobalRotations()) {
      break;
    }

    mapper.FilterRotations(
        init_mapper_options.max_relative_rotation_difference);
    ...
    while (reg_next_success) {
      ...
      const auto next_images = mapper.FindNextImages(options_->Mapper());

      if (next_images.empty() && reconstruction.NumRegImages() > 100) {
        const auto& unreg_image_ids = reconstruction.UnRegImageIds();
        if (unreg_image_ids.empty()) {
          std::cout << "  => All image have been registered." << std::endl;
          break;
        }

        size_t num_register_with_other_sensor = 0;

        for (const auto image_id : unreg_image_ids) {
          bool register_success = false;
          if (options_->register_image_with_rgbd) {
            PrintHeading1(StringPrintf("Registering image with RGBD #%d (%d)",
                                       image_id,
                                       reconstruction.NumRegImages() + 1));
            register_success =
                mapper.RegisterNextImageWithRGBD(options_->Mapper(), image_id);
          } else if (options_->register_image_with_arkit) {
            PrintHeading1(StringPrintf("Registering image with Arkit #%d (%d)",
                                       image_id,
                                       reconstruction.NumRegImages() + 1));
            register_success =
                mapper.RegisterNextImageWithArkit(options_->Mapper(), image_id);
          }

          if (register_success) {
            num_register_with_other_sensor++;
            Image& image = reconstruction.Image(image_id);
            TriangulateImage(*options_, image, &mapper);
            IterativeLocalRefinement(*options_, image_id, &mapper);
          }
        }

        // Run once global bundle adjustment
        if (num_register_with_other_sensor > 0) {
          AdjustGlobalBundle(*options_, &mapper);
          // Only run filter landmark
          FilterPoints(*options_, &mapper);
      ...
      for (size_t reg_trial = 0; reg_trial < next_images.size(); ++reg_trial) {
        ...
        if (!reg_next_success) {
          reg_next_success = mapper.RegisterNextImageWithArkit(
