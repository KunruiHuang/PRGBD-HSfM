...
inline Eigen::Matrix<T, 3, 3> hat(const Eigen::Matrix<T, 3, 1>& vec) {
  ...
  mat_hat << 0, -vec[2], vec[1], vec[2], 0, -vec[0], -vec[1], vec[0], 0;
  return mat_hat;
}
...
inline Eigen::Matrix<T, 3, 1> log(const Eigen::Matrix<T, 3, 3>& R) {
  ...
  if (w_length == 0.0f) {
    ...
    Eigen::Matrix<T, 3, 1> a(R(2, 1) - R(1, 2), R(0, 2) - R(2, 0),
                             R(1, 0) - R(0, 1));
...
inline Eigen::Matrix<T, 4, 4> exp(const Eigen::Matrix<T, 6, 1>& twist) {
  ...
  if (w_length == 0.0f) {
    ...
    Eigen::Matrix<T, 3, 3> V =
        ...
        (w_hat * w_hat) * (w_length - std::sin(w_length)) /
...
inline Eigen::Matrix<T, 6, 1> log(const Eigen::Matrix<T, 4, 4>& pose) {
  ...
  Eigen::Matrix<T, 3, 3> V_inv =
      Eigen::Matrix<T, 3, 3>::Identity() - 0.5f * w_hat +
      (1.0f - (w_length * std::cos(w_length * 0.5f)) /
                  (2.0f * std::sin(w_length * 0.5f))) *
