...
#include "rgbd/lie_algebra.h"
#include "rgbd/math.h"
#include "rgbd/normal_equations.h"

namespace colmap {

void Tracker::Config::print() const {
...
void Tracker::Config::validate() {
  // check pyramid levels
  if (num_levels < 1 || num_levels > 10) num_levels = 1;
  if (min_level < 0 || min_level >= num_levels) min_level = 0;
  if (max_level < 0 || max_level >= num_levels || max_level < min_level)
    max_level = num_levels - 1;
}

Tracker::Tracker(const Config& cfg, const CameraModel& cam)
    : cfg_(cfg),
...
const Tracker::Config& Tracker::config() const { return cfg_; }

bool Tracker::align(const FramePyramid& prev_pyramid,
  ...
  for (int lvl = cfg_.max_level; lvl >= cfg_.min_level; --lvl) {
    for (int itr = 0; itr < cfg_.num_iterations; ++itr) {
      ...
      Eigen::Matrix<float, 6, 1> xi_update =
          computeUpdate(prev_pyramid.at(lvl), cur_pyramid.at(lvl), lvl, xi);
...
void Tracker::calculateResiduals(const cv::Mat& gray_prev,
  ...
  for (int y = 0; y < gray_prev.rows; ++y) {
    for (int x = 0; x < gray_prev.cols; ++x) {
      ...
      if (pt3_cur[2] <= 0.0f) continue;

      // project 3d point to 2d and sample intensity
      float val_cur = interpolate(gray_cur, cam.project(pt3_cur));
...
Eigen::Matrix<float, 6, 1> Tracker::calculateJacobian(
  ...
  // analytic partial derivates (for more details see Christian Kerl's master's
  // thesis)
  ...
  jacobian[3] =
      -(fg_x * pt3_cur[0] * pt3_cur[1]) * pt3_z_inv_sqr -
      fg_y * (1 + (pt3_cur[1] * pt3_z_inv) * (pt3_cur[1] * pt3_z_inv));
  jacobian[4] =
      fg_x * (1.0f + (pt3_cur[0] * pt3_z_inv) * (pt3_cur[0] * pt3_z_inv)) +
      (fg_y * pt3_cur[0] * pt3_cur[1]) * pt3_z_inv_sqr;
  jacobian[5] = (-fg_x * pt3_cur[1] + fg_y * pt3_cur[0]) * pt3_z_inv;
  return jacobian;
}

float Tracker::calculateResidualWeight(const float residual,
  ...
  if (cfg_.use_huber_weights) {
    ...
    w *= calculateHuberWeight(residual, huber_weights_k);
  }
  if (cfg_.use_depth_weighting) {
    // integrate depth into weight (depth certainty decreases quadratically with
    // depth)
    w /= (depth * depth);
  }
  return w;
...
Eigen::Matrix<float, 6, 1> Tracker::computeUpdate(
  ...
  const float residuals_stddev = calculateStdDev(residuals);
  ...
  for (int y = 0; y < prev_frame.height(); ++y) {
    for (int x = 0; x < prev_frame.width(); ++x) {
      ...
      const float g_x = interpolate(cur_frame.gradientX(), pt2_cur);
      const float g_y = interpolate(cur_frame.gradientY(), pt2_cur);
      if (!std::isnan(g_x) && !std::isnan(g_y)) {
        // compute Jacobian row, residual and residual weight
        Eigen::Matrix<float, 6, 1> jacobian_row =
            calculateJacobian(g_x, g_y, cam, pt3_cur);
        float residual = residuals.at<float>(y, x);
        float residual_weight =
            calculateResidualWeight(residual, residuals_stddev, d_prev);
